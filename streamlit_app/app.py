    ...
    if st.button("Start Analysis", use_container_width=True, disabled=st.session_state.session_id is not None):
        if not user_id:
            ...
            with st.spinner("Starting analysis..."):
                result = call_backend("/api/chat/start", method="POST", payload={"user_id": user_id, "problem_description": problem_description.strip()})
            if "error" in result:
        ...
        if not problem_id:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Validating your solution..."):
                    validation_result = call_backend("/api/solutions/validate", method="POST", payload={"user_solution": prompt, "problem_id": problem_id})
                if "error" in validation_result:
                    ...
                    if status == "matches_existing":
                        existing_count = validation_result.get("existing_solutions_count", 1)
                        ...
                        st.markdown(f"\n\n{confirmation_msg}")
                        st.session_state.messages.append({"role": "assistant", "content": f"{response_msg}\n\n{confirmation_msg}"})
